...
var cachedDeviceMessages = new Array();
...
function mongoConnected(err, db){
    ...
    zte_Broker.onMessage((gatewayName, topic, message, packet) => {
        ...
        var messageId = json_object.messageId;

        if(messageId){
            var messageCachedKey = "OBDless-" + deviceId + "-" + messageId;

            // redisClient.exists(cachedKey, function(exist){
            //     if(!exist){
            //         redisClient.set(cachedKey, true, 'EX', 60 * 60 * 24, function(status){});
            //         processMessageData(topic, json_object);
            //     }
            // });

            if(cachedDeviceMessages[messageCachedKey]){
                console.log('#################Duplicated frame returned, ignore this frame...#################');
                console.log('deviceId : ' + deviceId);
                console.log('topic : ' + topic);
                console.log('frameId : ' + messageId);
                console.log('#################Duplicated frame returned, ignore this frame...#################');
                return;
            }

            cachedDeviceMessages[messageCachedKey] = true;
        }

        processMessageData(topic, json_object);

        if(cachedDeviceMessages.length >= 10000){
            cachedDeviceMessages.splice(0, 1000);
        }
...
function processMessageData(topic, json_object){
