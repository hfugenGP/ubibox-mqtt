...
Broker.prototype.subscribeOne = function(topic) {
    this.client.subscribe(topic, function(err, granted) {
        if (err) {
            console.log('False to subscribe to topic "' + topic + '" on broker ' + this.name);
            console.log(err);
        }

        if (granted) {
            console.log('Subscribed to topic "' + topic + '" on broker ' + this.name);
        }
    })
}
...
Broker.prototype.unsubscribeOne = function(topic) {
    this.client.unsubscribe(topic, function(err) {
        if (err) {
            console.log('False to unsubscribe to topic "' + topic + '" on broker ' + this.name);
            console.log(err);
        } else {
            console.log('Unsubscribed to topic "' + topic + '" on broker ' + this.name);
        }
    })
}
