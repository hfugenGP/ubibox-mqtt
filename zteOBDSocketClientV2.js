...
// const ZTEDataServiceV3 = require('./services/zteDataServiceV2');
...
var fabrick_gateway = {
    id: "Fabrick ZTE Sockets V2 Client " + config.zteBroker.idKey,
    host: config.zteBroker.host,
    port: config.zteBroker.port,
    topics: { 'config/ztewelink/portal/Device/V2/Message': 1, 'config/ztewelink/portal/Devices/V2': 1 }
};
...
function mongoConnected(err, db){
    ...
    var zteDataSenderService = new ZTEDataServiceV3(mongodb, redisClient, cachedDeviceAlert);
...
function handleDeviceConnetion(sock) {
    ...
    sock.on('data', function(data) {
        var zteDataService = new ZTEDataServiceV3(mongodb, redisClient, cachedDeviceAlert);
